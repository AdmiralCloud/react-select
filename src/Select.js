...
class Select extends React.Component {
    ...
    toggleTouchOutsideEvent(enabled) {
        var eventTogglerName = enabled
            ...
            (document.removeEventListener
                ? 'removeEventListener'
                : 'detachEvent');
        var pref             = document.addEventListener
            ? ''
            : 'on';
    ...
    handleInputFocus(event) {
        ...
        toOpen     = this._focusAfterClear
            ? false
            : toOpen;  //if focus happens after clear values, don't open dropdown yet.
    ...
    getValueArray(value, nextProps = undefined) {
        ...
        const props = typeof nextProps === 'object'
            ? nextProps
            : this.props;
        if (props.multi) {
    ...
    popValue() {
        ...
        this.setValue(this.props.multi
    ...
    focusAdjacentOption(dir) {
        ...
        if (!this.state.isOpen) {
            const newState = {
                focusedOption: this._focusedOption || (options.length
                    ? options[dir === 'next'
                        ? 0
    ...
    render() {
        let valueArray = this.getValueArray(this.props.value);
        let options    = this._visibleOptions = this.filterOptions(this.props.multi && this.props.removeSelected
            ? valueArray
            : null);
            ...
            <div ref={ref => this.wrapper = ref}
                ...
                { // AC change
                    this.props.multiSelectListBelow && valueArray.length > 0
                        ? (
                            <div className={multiSelectListStyle}>
                                {
