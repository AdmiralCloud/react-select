  ...
  focusAdjacentOption (dir) {
    ...
    if(!this.state.isOpen) {
      this.setState({
        isOpen:        true,
        inputValue:    '',
        focusedOption: this._focusedOption || options[dir === 'next'
          ? 0
  ...
  filterOptions (excludeOptions) {
    ...
    if(typeof this.props.filterOptions === 'function') {
      ...
      return options.filter(option => {
        if(excludeOptions && excludeOptions.indexOf(option[this.props.valueKey]) > -1) {
          return false;
        }
        if(this.props.filterOption) {
          return this.props.filterOption.call(this, option, filterValue);
        }
        if(!filterValue) {
          return true;
        }
        ...
        if(this.props.ignoreAccents) {
          if(this.props.matchProp !== 'label') {
            valueTest = stripDiacritics(valueTest);
          }
          if(this.props.matchProp !== 'value') {
